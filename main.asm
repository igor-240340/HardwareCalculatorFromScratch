...
;            .EQU EMULKEYPD=1                ; транслирует их в коды энкодера MM74C922, имитируя нажатие кнопок.
            ...
            .EQU LCDLEN=16                  ; Максимальная длина видимой строки в LCD.
;            .EQU LCDLEN=40                  ; Тестовое увеличение лимита для проверки обработки переполнения в ATOF.
            ...
            .DEF LCDLIM=R3                  ; Текущее максимальное количество выводимых на LCD символов (может меняться в зависимости от контекста).
...
KEYMAP:     .BYTE 16                        ; 0x07F0. Таблица ASCII-символов нажатых клавиш.
...
S0:         LDI R16,'C'                     ;
            ...
            AND R16,KEY                     ; Другие возможные клавиши: '.','+','*','/', имеют старший полубайт равный 0x20.
            LDI R17,0x30                    ;
            EOR R16,R17                     ; KEY=['1','9']?
            BREQ S0DIG19                    ;
...
.ENDIF
            ...
            ; когда для промежуточных вычислений одинарной точности становится недостаточно.
            ;
...
OP1FLTERR0: POP R16                         ; Выбрасываем из стека адрес возврата после ATOF.
            ...
            POP RETL                        ;
            POP KEY                         ; Перед FTOA мы сохраняли KEY - выбрасываем его.
.IFDEF EMULKEYPD
...
S4DIG09:    MOV R16,S                       ; Если S в текущем состоянии оказался на единицу меньше LCDLIM, то в следующем состоянии он будет равен LCDLIM.
...
S5:         LDI R16,'C'                     ;
            ...
            AND R16,KEY                     ; поскольку она тоже имеет 0x20 в ASCII в старшем полубайте - 
...
.ENDIF
            ...
            LDI ZL,LOW(KEYMAPPRG<<1)        ; Копируем таблицу ASCII-кодов клавиш из памяти программ в SRAM.
...
MAIN:       NOP
            ...
            LDI R16,0                       ; Количество введенных символов S=0.
            MOV S,R16                       ;

            LDI R16,1                       ; Начинаем ввод с первого операнда P=1.
            MOV P,R16                       ;
...
;            MOV LCDLIM,R16                  ;
            ...
            LDI ZL,LOW(KEYMAPPRG << 1)      ; NOTE: LSB адреса в памяти программ используется для идентификации номера байта: первый, второй (см. даташит).
            LDI ZH,HIGH(KEYMAPPRG << 1)     ;
READ0:      LPM R4,Z+                       ;
...
; Примеры определены в [State Graph and Test Examples.xmind].
; сначала подряд идут "зелёные" примеры, содержащие корректные числовые строки.
