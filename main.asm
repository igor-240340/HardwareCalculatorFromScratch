...
;            .EQU TESTINPUT=1                ; РЕЖИМ ТЕСТИРОВАНИЯ ВВОДА С КЛАВИАТУРЫ.
            ...
            .EQU PREC=5                     ; КОЛИЧЕСТВО ЗНАКОВ ПОСЛЕ ТОЧКИ ДЛЯ FTOA.
...
.IFDEF TESTINPUT
            ...
            POP RETL                        ;
            POP R17                         ; БЭКАПИМ АДРЕС ТЕКУЩЕГО СОСТОЯНИЯ.
            POP R16                         ;
...
.IFDEF TESTINPUT
            ...
            POP ZL                          ;
.ENDIF
            LDI R16,LOW(SHOWRES)            ; РЕЗУЛЬТАТ ВЫЧИСЛЕН, НОВОЕ СОСТОЯНИЕ - ПОКАЗ РЕЗУЛЬТАТА.
...
;TESTNUM:  .DB "/",0
            ...
            ; ПРИМЕР 1. АРИФМЕТИЧЕСКИЕ ВЫРАЖЕНИЯ.
;TESTNUM:  .DB "0.5*0.6+",0
;TESTNUM:  .DB "1+3+",0
TESTNUM:  .DB "-5+3+",0
.ENDIF